...
app.post('/auth', (request, response) => {
	...
	if (username && password) {
		connection.query('SELECT * FROM `new_schema`.`acc` WHERE username = ? AND password = ?', [username, password], function(error, results, fields) {
			if (results.length > 0) {
...
app.get('/pagrindinis', (req, res) => {
	if (req.session.loggedin) {
		connection.query('select * from new_schema.taskas as t inner join new_schema.gauta as g where g.pavadinimas = t.pavadinimas;', function(error, results, fields) {
            if (error) throw error;
...
app.get('/apskaita/:id', (req, res) => {
    if (req.session.loggedin) {
		...
		connection.query('select * from new_schema.taskas as t inner join new_schema.gauta as g inner join new_schema.nuimta as n inner join new_schema.nuimta_pradzia as p where g.pavadinimas = (?) and t.pavadinimas = (?) and n.pavadinimas = (?) and p.pavadinimas = (?) and n.id = p.id', 
		[id, id, id, id], 
		function(error, result) {
			...
			if (result.length > 0) {
				...
				connection.query('select * from new_schema.taskas as t inner join new_schema.gauta as g where g.pavadinimas = (?) and t.pavadinimas = (?)', [id, id], function(error, result) {
					res.render('apskaita', {result: result});
					console.log(id);
				});
			}
...
app.post('/apskaita/:id', (req, res) => {
	...
	const id = req.params.id;
	connection.query(
		'UPDATE `new_schema`.`taskas` SET minkstiledai = minkstiledai + (?), kabinamiledaivnt = kabinamiledaivnt + (?), kabinamiledai = kabinamiledai + (?), serbetas = serbetas + (?), stiklines02 = stiklines02 + (?), stiklines03 = stiklines03 + (?), stiklines05 = stiklines05 + (?), kava = kava + (?), vafliutesla = vafliutesla + (?), gerimai1 = gerimai1 + (?), gerimai15 = gerimai15 + (?), gerimai2 = gerimai2 + (?) WHERE tasko_ID = (?)', 
	...
	connection.query(
		'UPDATE `new_schema`.`atvezta` SET atveztaMinksti = atveztaMinksti + (?), atveztaKabinamiledaivnt = atveztaKabinamiledaivnt + (?), atveztaKabinami = atveztaKabinami + (?), atveztaSerbetas = atveztaSerbetas + (?), atveztaStiklines02 = atveztaStiklines02 + (?), atveztaStiklines03 = atveztaStiklines03 + (?), atveztaStiklines05 = atveztaStiklines05 + (?), atveztaKava = atveztaKava + (?), atveztaVafliutesla = atveztaVafliutesla + (?), atveztaGerimai1 = atveztaGerimai1 + (?), atveztaGerimai15 = atveztaGerimai15 + (?), atveztaGerimai2 = atveztaGerimai2 + (?) WHERE tasko_ID = (?)', 
	[pilstomiLedai, kabinamiLedaiVnt, kabinamiLedai, serbetas, stiklines02, stiklines03, stiklines05, kava, vafliuTesla, gerimai, gerimai15, gerimai2, id], 
	function(error, result) {
		if(error) throw error;
		console.log(id);
...
app.post('/nuemimas/:id', (req,res) => {
	...
	const id = req.params.id;
	connection.query(
		'INSERT INTO new_schema.nuimta (nuimtaMinksti, nuimtaKabinamiledaivnt, nuimtaKabinami, nuimtaSerbetas, nuimtaStiklines02, nuimtaStiklines03, nuimtaStiklines05, nuimtaKava, nuimtaVafliutesla, nuimtaGerimai1, nuimtaGerimai15, nuimtaGerimai2, pavadinimas, data) VALUES ((?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?));',
	...
	connection.query(
		'insert into new_schema.nuimta_pradzia (nuimta_pradziaMinksti, nuimta_pradziaKabinamiledaivnt, nuimta_pradziaKabinami, nuimta_pradziaSerbetas, nuimta_pradziaStiklines02, nuimta_pradziaStiklines03, nuimta_pradziaStiklines05, nuimta_pradziaKava, nuimta_pradziaVafliutesla, nuimta_pradziaGerimai1, nuimta_pradziaGerimai15, nuimta_pradziaGerimai2, pavadinimas) select atveztaMinksti, atveztaKabinamiledaivnt, atveztaKabinami, atveztaSerbetas, atveztaStiklines02, atveztaStiklines03, atveztaStiklines05, atveztaKava, atveztaVafliutesla, atveztaGerimai1, atveztaGerimai15, atveztaGerimai2, pavadinimas from new_schema.atvezta where new_schema.atvezta.tasko_ID = (?)',
		[id], function(error, result) {
			if (error) throw error;
			console.log(result);
		});
	connection.query('UPDATE `new_schema`.`atvezta` SET atveztaMinksti = 0, atveztaKabinamiledaivnt = 0, atveztaKabinami = 0, atveztaSerbetas = 0, atveztaStiklines02 = 0, atveztaStiklines03 = 0, atveztaStiklines05 = 0, atveztaKava = 0, atveztaVafliutesla = 0, atveztaGerimai1 = 0, atveztaGerimai15 = 0, atveztaGerimai2 = 0 WHERE tasko_ID = (?)',
	[id], function(error, result) {
		if (error) throw error;
		console.log(result);
	});
